...
def juego_principal():
    ...
    for _ in range(21):  # 21 pantallas
        letras_seleccionadas = random.sample(letras, 7)
        mostrar_letras(letras_seleccionadas)
        chequear_salida()
        
        # Tomar la siguiente posición de la lista aleatorizada de preguntas
        posicion_pregunta = preguntas_posicion.pop()
        letra_pregunta = letras_seleccionadas[posicion_pregunta]  # Obtener la letra en esa posición
        
        # Pregunta sobre la letra sin mencionar la posición
        pregunta = f"¿Estaba la letra {letra_pregunta}? (derecha para sí, izquierda para no)"
        texto_pregunta = visual.TextStim(win, text=pregunta, height=0.09)
        texto_pregunta.draw()
        win.flip()
