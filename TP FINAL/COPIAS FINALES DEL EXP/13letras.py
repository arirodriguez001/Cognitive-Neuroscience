...
def mostrar_enfoque_deslizante(serie_letras):
    ...
    total_espacio = 1  # Espacio total para letras                                          # !!!!
    x_start = -total_espacio / 2  # Centro de la ventana menos la mitad del espacio total
    espacio = total_espacio / (13 - 1)  # Espacio distribuido entre 5 letras
...
def pantalla_prueba():
    fixation = visual.ShapeStim(
				win, name='polygon', vertices='cross',
			    size=(0.1, 0.15),
			    ori=0, pos=(0, 0),
    ...
    frs = 90
    fixtime=0.5
    fixation_dur =  int(random.gauss(fixtime,0.2)*frs)
    
    for _ in range(5):
        for s in range(int(fixation_dur)):
            fixation.draw()
            win.flip()
        letras_seleccionadas = random.sample(letras, 13)  # !!!!
...
def generar_preguntas_combinadas():
    posiciones = list(range(13))  # cantidad de posiciones/letras                   # !!!!
    ...
    # Crear 26 preguntas sobre letras presentes en la serie
    for _ in range(26):                                         # !!!!
        letras_seleccionadas = list(random.sample(letras, 13))  # !!!!         
    ...
    # Crear  preguntas sobre letras no presentes en la serie
    for _ in range(9):                                      # !!!!                                         
        letras_seleccionadas = random.sample(letras, 13)    # !!!!
    ...
    # Aleatorizar todas las preguntas (26+9)
...
dlg = gui.Dlg(title="Experimento 13-letras") # !!!!
dlg.addField('archivo', 'Nombre y edad (ej: Laura20)')
...
PARTICIPANT_FILENAME = 'C:/Users/Admin/Desktop/EXPERIMENTO/Respuestas13letras/' + 'data13_' + str(ARCHIVO) + '.csv' # !!!!

# Correr el experimento
win = visual.Window([1366, 768], fullscr=True, monitor='testMonitor', units='norm', color=[-1, -1, -1])
mostrar_instrucciones(
    "Lea atentamente."
    "\n\n En este experimento se le mostrarán 13 letras en la pantalla y luego"# !!!!
    ...
    "El procedimiento se repetirá sucesivamente treinta y cinco.\n"# !!!!
